...
class Cell(nn.Module):
    def __init__(self, ninp, nhid, translation, genotype):
        ...
        self._SigmoidWeight = nn.Parameter(
            torch.Tensor((input_multiplier * ninp + nhid), 1).uniform_(-INIT_RANGE, INIT_RANGE))
        ...
        self._W0_rev = nn.Parameter(torch.Tensor((input_multiplier * ninp + nhid), 2 * nhid).uniform_(-INIT_RANGE, INIT_RANGE))
        self._Ws_rev = nn.ParameterList([
            nn.Parameter(torch.Tensor(nhid, 2 * nhid).uniform_(-INIT_RANGE, INIT_RANGE)) for _ in range(steps)
        ])
    ...
    def _compute_init_state(self, x, h_prev):
        ...
        sigmoid_weights = torch.sigmoid(xh_prev.mm(self._SigmoidWeight))
        xh_prev = self.bn_in(xh_prev*sigmoid_weights)
        ...
        sigmoid_weights_rev = torch.sigmoid(xh_prev.mm(self._SigmoidWeight))
        xh_prev_rev = self.bn_in_rev(xh_prev_rev*sigmoid_weights_rev)
        c0_rev, h0_rev = torch.split(xh_prev_rev.mm(self._W0_rev), self.nhid, dim=-1)
    ...
    def cell(self, x, h_prev):
        ...
        for i, (name, pred) in enumerate(self.genotype.recurrent):
            ...
            ch_rev = s_prev[:, s_prev.shape[1] // 2:].contiguous().mm(self._Ws_rev[i])
    ...
    def cell(self, x, h_prev):
        ...
        for i in range(STEPS):
            ...
            ch_rev = states[:, :, states.shape[2] // 2:].contiguous().view(
                -1, self.nhid
            ).mm(self._Ws_rev[i]).view(i + 1, -1, 2 * self.nhid)
